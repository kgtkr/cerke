...
let useScheduledTransitioning = (
  ~elRef: React.ref<Js.Nullable.t<Webapi.Dom.Element.t>>,
  ~scheduledProps: Relude.NonEmptyArray.t<'a>,
  ~changeScheduledProps: Relude.NonEmptyArray.t<'a> => unit,
  ~timeoutMs: int=1000,
  _: unit,
): 'a => {
  let transitioningRef = React.useRef(false)

  let (currentProps, propsList) = scheduledProps->Relude.NonEmptyArray.uncons
  ...
  if firstRun {
    ...
    Relude.Array.head(propsList)->Option.getWithDefault(currentProps)
  }
}
